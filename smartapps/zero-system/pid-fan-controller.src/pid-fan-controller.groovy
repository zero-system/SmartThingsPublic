...
int setFan(double rawLevel) {
    int boundedLevel = state.fanLevel

    if (rawLevel < 0) boundedLevel = 0
    else if (rawLevel >= 0 && rawLevel < minFanLevel) boundedLevel = minFanLevel // If calculated output is below the min threshold, set level to minLevelOutput.
    else if (rawLevel > 100) boundedLevel = 100
    else boundedLevel = (int) Math.round(rawLevel) // Else the output should be rounded to integer.
...
double getTemp() {
    ...
    if (state.numTempSensors == 1)
        temp = tempSensors[0].currentValue("temperature")

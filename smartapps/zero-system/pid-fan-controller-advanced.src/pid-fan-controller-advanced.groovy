...
definition(
        ...
        description: "PID Fan Control. \r\nThis app varies a fan(s) speed, by using a dimmer. \r\nThe app is able to " +
                "maintain a constant room temperature even if a heat source present, e.g. a computer.\r\nAdditional " +
            ...
            page( name: "settingsScheduling" , title: "Scheduling Settings" , nextPage: "settingsActive" )
                    {
                        ...
                        section( "Luminance Control" , hideable: true , hidden: true )
                                {
            ...
            page( name: "settingsActive" , title: "Active Temperature Control Settings" , nextPage: "settingsSafeguard" )
                    {
                        ...
                        section( "Active Cooling Control Settings" , hideable: true , hidden: true )
                                {
                                    paragraph "If enabled, will switch ON cooling device, outside \"PID Time Frame\""
                                    input( title: "Enable Active Cooling" , name: "enableActiveCooling" , type: "bool" , required: true , defaultValue: false )

                                    paragraph "Select the cooling device(s), that will cool the room."
                                    input( title: "Cooling Device(s)" , name: "activeCoolingDevices" , type: "capability.switch" , multiple: true , required: false )

                                    paragraph "Enable A/C thermostat control. Will switch A/C ON/OFF to control temperature, to reach \"Target Temperature\". Will be disabled if temperature reaches \"Minimum Temperature\""
                                    input( title: "Enable A/C Thermostat Control" , name: "enableCoolingControl" , type: "bool" , required: false , defaultValue: false )
                                }
                        section( "Active Heating Control Settings" , hideable: true , hidden: true )
                                {
                                    paragraph "If enabled, will switch ON heating device, outside \"PID Time Frame\""
                                    input( title: "Enable Active Heating" , name: "enableActiveHeating" , type: "bool" , required: true , defaultValue: false )

                                    paragraph "Select the heating device(s), that will heat the room."
                                    input( title: "Heating Device(s)" , name: "activeHeatingDevices" , type: "capability.switch" , multiple: true , required: false )

                                    paragraph "Enable heater thermostat control. Will switch heater ON/OFF to control temperature, to reach \"Target Temperature\". Will be disabled if temperature reaches \"Maximum Temperature\""
                                    input( title: "Enable Heater Thermostat Control" , name: "enableHeatingControl" , type: "bool" , required: false , defaultValue: false )
                                }
                    }
...
void scheduledHandler()
{
    ...
    if ( settings.enableTimeFrame )
    {
        ...
        boolean withinTimeFrame = timeOfDayIsBetween( settings.startTime , settings.stopTime , currentTime , location
                .timeZone )

        log.debug "> scheduledHandler: TIME( withinTimeFrame: $withinTimeFrame )"
        if ( withinTimeFrame )
        {
            if ( activeTempControlEnabled() ) activeTempControlOFF()
            pidControlON()
        }
        else
        {
            pidControlOFF()
            if ( activeTempControlEnabled() ) activeTempControlON()
        }
    }
    else
    {
...
void pidControlON()
{
    ...
    {
        log.warn "--> pidFreezingProtection"
        pidFreezingProtection()
    }
}

boolean activeTempControlEnabled()
{
    return ( settings.enableActiveCooling || settings.enableActiveHeating )
}
...
void activeTempControlOFF()
{
    ...
    if ( settings.enableActiveCooling )
    {
        log.info "--> activeCooling(OFF)"
        activeCooling( false )
    }

    if ( settings.enableActiveHeating )
    {
        log.info "--> activeHeating(OFF)"
        activeHeating( false )
    }
...
void activeTempControlON()
{
    ...
    // TODO: Implement - Depending on outside temp, cool to minTemp or heat to maxTemp
    // Current implementation relies on user to turn off one or the other
...
void activeCooling( boolean on , boolean logging = false )
{
    ...
    if ( on )
    {
        ...
        {
            if ( logging ) log.debug "activeCooling(ON): activeCoolingDevices(ON)"
            state.lastCoolingState = setSwitch( ON() , settings.activeCoolingDevices , state.lastCoolingState )
        }
    }
    ...
    {
        if ( logging ) log.debug "activeCooling(OFF): activeCoolingDevices(OFF)"
        state.lastCoolingState = setSwitch( OFF() , settings.activeCoolingDevices , state.lastCoolingState )
    }

}

void activeHeating( boolean on , boolean logging = false )
{
    // turn ON active heating
    if ( on )
    {
        // max temp shutoff
        if ( getTemp() >= settings.maxTemp - 1 ) // prevent tripping overHeatProtection
        {
            // TODO: Make this more robust. Needs to be OFF until temp reaches target
            log.warn "activeHeating(ON): maxTemp - triggered"
            state.lastHeatingState = setSwitch( OFF() , settings.activeHeatingDevices , state.lastHeatingState )
        }

        // heating control is enabled
        else if ( settings.enableHeatingControl )
        {
            // turn OFF heating device when currentTemp reaches targetTemp
            if ( getTemp() >= settings.targetTemp )
            {
                if ( logging ) log.debug "activeHeating(ON) -> enableHeatingControl(TRUE): activeHeating(OFF)"
                state.lastHeatingState = setSwitch( OFF() , settings.activeHeatingDevices , state.lastHeatingState )
            }

            // turn ON heating device when currentTemp is below targetTemp
            else if ( getTemp() < settings.targetTemp )
            {
                if ( logging ) log.debug "activeHeating(ON) -> enableHeatingControl(TRUE): activeHeating(ON)"
                state.lastHeatingState = setSwitch( ON() , settings.activeHeatingDevices , state.lastHeatingState )
            }
        }

        // when heating control is disabled, just turn on and leave on heating. Unless maxTemp was reached.
        else
        {
            if ( logging ) log.debug "activeHeating(ON): activeHeatingDevices(ON)"
            state.lastHeatingState = setSwitch( ON() , settings.activeHeatingDevices , state.lastHeatingState )
        }
    }
    // turn OFF active heating
    else
    {
        if ( logging ) log.debug "activeHeating(OFF): activeHeatingDevices(OFF)"
        state.lastHeatingState = setSwitch( OFF() , settings.activeHeatingDevices , state.lastHeatingState )
    }
...
int setFanLevel( double rawLevel , boolean logging = false )
{
    ...
    // Prevent constant commands being sent to controller if no change is detected. If the level has changed (manually) reset it (Connor Proof)
...
void triggerAlert( String alertMessage , String thrownFrom )
{
    ...
    {
        sendPush( getLabel() + " " + alertMessage )
        state.lastAlertTime = getTime()
    }
    log.warn( thrownFrom + ": " + getLabel() + " " + alertMessage )
...
void pidOverheatProtection()
{
    ...
    else if ( returnToTargetTemp || afterTimeDuration )
    {
        ...
        log.warn "pidOverheatingProtection -> enableProtection(TRUE) - > maxTempFlag(FALSE): overheatDevices(ON)"
    }
    else
        log.warn "pidOverheatingProtection: returnToTargetTemp($returnToTargetTemp) , afterTimeDuration" + "($afterTimeDuration)"
}

// If freezing protection occurs, it is because the outside air is too cold. Turn off fans and let heating sources (lights or heaters) heat the space back up to target temp.
void pidFreezingProtection()
{
    // Turn Off Fans.
    setFanLevel( 0 )
    log.info "--> setFanLevel(0)"

    // Turn ON Heating, if there is heating devices.
    if ( settings.enableActiveHeating ) activeHeating( true )

    // Temp return to normal
    if ( getTemp() >= settings.targetTemp )
    {
        // Turn OFF Heating, if there is heating devices.
        if ( settings.enableActiveHeating ) activeHeating( false )

        // clear warnings
        state.minTempFlag = false
        log.warn "pidFreezingProtection -> enableProtection(TRUE) - > minTempFlag(FALSE)"
    }
}